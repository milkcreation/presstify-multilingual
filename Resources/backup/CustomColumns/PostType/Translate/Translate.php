...
class Translate extends \tiFy\Components\CustomColumns\Factory
{
    ...
    {
        add_action('wp_ajax_tiFyMultilingualPostColumn', [$this, 'wp_ajax']);
    }

    /** == Mise en file des scripts de l'interface d'administration == **/
    public function admin_enqueue_scripts()
    {
        wp_enqueue_style('tiFyMultilingualCustomColumnsPostTypeTranslate',
            self::tFyAppUrl(get_class()) . '/Translate.css', [], 170310);
        wp_enqueue_script('tiFyMultilingualCustomColumnsPostTypeTranslate',
            self::tFyAppUrl(get_class()) . '/Translate.js', ['jquery'], 170310, true);
    }
    ...
    public function content($column, $post_id)
    {
        ...
        foreach (Multilingual::getSites() as $site) :
            ...
            $translate_id = ($translate_id = get_post_meta($post_id, '_translate_blog_' . $site->blog_id,
                true)) ? $translate_id : 0;
            $href = !$translate_id ? wp_nonce_url(add_query_arg(['post'   => $post->ID,
                                                                 'lang'   => $site->blog_id,
                                                                 'action' => 'tiFyMultilingualDuplicatePost',
            ], admin_url(sprintf($post_type_object->_edit_link, $post->ID))),
                'tify_multilingual_duplicate_post:' . $post->ID) : '';
            ...
            $output .= Multilingual::getFlag($site->blog_id, ['width' => '25', 'height' => 'auto']);
            $output .= "\t\t\t<strong>" . ($translate_id ? __('Voir le détail', 'tify') : __('Créer une copie',
                    'tify')) . "</strong>";
            $output .= "\t\t</a>\n";
    ...
    public function wp_ajax()
    {
        ...
        if (!$post = get_post($post_id)) {
            wp_send_json_error(['error' => __('Contenu indisponible', 'tify')]);
        }
